...
def fossil_adder(inputName, REOPT_FOLDER, FOSSIL_OVER_CRIT_KW, max_crit_load):
	''' Calculate total fossil kW needed to meet max_crit_load at all hours of the year
	plus a user-inputted design safety factor equal to (1+FOSSIL_OVER_CRIT_KW)*max_crit_load'''
	reopt_out = json.load(open(REOPT_FOLDER + inputName))
	mg_num = 1
	fossil_total_REopt = reopt_out.get(f'sizeDiesel{mg_num}', 0.0)
	if max_crit_load >= fossil_total_REopt:
		fossil_over_crit_calc = max_crit_load*(1+FOSSIL_OVER_CRIT_KW)
	elif max_crit_load < fossil_total_REopt:
		if fossil_total_REopt >= max_crit_load*(1+FOSSIL_OVER_CRIT_KW):
			fossil_over_crit_calc = fossil_total_REopt
		else:
			fossil_over_crit_calc = max_crit_load*(1+FOSSIL_OVER_CRIT_KW)
	print(fossil_over_crit_calc,"kW fossil_over_crit_calc is", round(100*(fossil_over_crit_calc-fossil_total_REopt)/fossil_total_REopt), "% more kW fossil than recommended by REopt for", REOPT_FOLDER)
	return fossil_over_crit_calc

def get_gen_ob_from_reopt(REOPT_FOLDER, fossil_over_crit_calc, diesel_total_calc=False):
	...
	# Unused code for use with DIESEL_SAFETY_FACTOR
	...
	# code to be used with FOSSIL_OVER_CRIT_KW
	if fossil_over_crit_calc == False:
		fossil_size_total = reopt_out.get(f'sizeDiesel{mg_num}', 0.0)
	else:
		fossil_size_total = fossil_over_crit_calc
	fossil_size_existing = reopt_out.get(f'sizeDieselExisting{mg_num}', 0.0)
	if fossil_size_total - fossil_size_existing > 0:
		fossil_size_new = fossil_size_total - fossil_size_existing
	else:
...
def feedback_reopt_gen_values(BASE_NAME, LOAD_NAME, REOPT_INPUTS, REOPT_FOLDER_BASE, REOPT_FOLDER_FINAL, microgrid, critical_load_percent, fossil_over_crit_calc, diesel_total_calc=False):
	...
	if not os.path.isdir(REOPT_FOLDER_FINAL):
		...
		gen_sizes = get_gen_ob_from_reopt(REOPT_FOLDER_BASE, fossil_over_crit_calc, diesel_total_calc=False)
		...
		if battery_pow_new > 0:
			if fossil_over_crit_calc == False:
				...
				allInputData['batteryKwhExisting'] = 0
				allInputData['battery'] = 'on'
			# if the fossil_over_crit_calc variable is enabled, let the battery optimize afresh, removing any existing battery
			# Following else clause works with 1mg and 4mgs test files:
			else:
				allInputData['batteryPowerMin'] = 0
				allInputData['batteryPowerMax'] = 100000
				allInputData['batteryKwExisting'] = 0
				# If recommended battery power is larger than existing, then let rerun of REopt suggest the best capacity of the battery instead of tying it to the original existing kwh
				allInputData['batteryCapacityMin'] = 0
				allInputData['batteryCapacityMax'] = 100000
			...
			# # following else clause for oro grande mg to set both diesel and battery
			# else:
			# 	allInputData['batteryPowerMin'] = battery_pow_total
			# 	allInputData['batteryPowerMax'] = battery_pow_total
			# 	allInputData['batteryKwExisting'] = 0
...
def build_new_gen_ob_and_shape(REOPT_FOLDER, GEN_NAME, microgrid, BASE_NAME, mg_name, fossil_over_crit_calc, diesel_total_calc=False):
	...
	gen_sizes = get_gen_ob_from_reopt(REOPT_FOLDER, fossil_over_crit_calc, diesel_total_calc=False)
	...
	battery_cap_existing = gen_sizes.get('battery_cap_existing')
	battery_pow_total = gen_sizes.get('battery_pow_total')
	# print("build_new)gen_ob_and_shape() battery_pow_total:", battery_pow_total)
	battery_pow_new = gen_sizes.get('battery_pow_new')
	battery_pow_existing = gen_sizes.get('battery_pow_existing')
	...
	for gen_ob_existing in gen_obs_existing:
		...
		if gen_ob_existing.startswith('battery_'):
			...
			# if no new battery has been built, existing battery takes the full battery load, using 0-1 scale
				...
				#HACK: Use of fossil_over_crit_calc Forcibly sets loadshape of any existing battery gen to 0 for the purposes of using the argument fossil_over_crit_calc
	 				gen_df_builder[f'{gen_ob_existing}'] = pd.Series(np.zeros(8760))
	gen_df_builder.to_csv(GEN_NAME, index=False)
	return gen_obs

def gen_existing_ref_shapes(REF_NAME, REOPT_FOLDER_BASE, REOPT_FOLDER_FINAL, fossil_over_crit_calc):
	...
	gen_sizes = get_gen_ob_from_reopt(REOPT_FOLDER_FINAL, fossil_over_crit_calc, diesel_total_calc=False)
...
def microgrid_report_csv(inputName, outputCsvName, REOPT_FOLDER, microgrid, mg_name, max_crit_load, ADD_COST_NAME, fossil_over_crit_calc, diesel_total_calc=False):
...
def microgrid_report_list_of_dicts(inputName, REOPT_FOLDER, microgrid, mg_name, max_crit_load, ADD_COST_NAME, fossil_over_crit_calc, diesel_total_calc=False):
...
def main(BASE_NAME, LOAD_NAME, REOPT_INPUTS, microgrid, playground_microgrids, GEN_NAME, REF_NAME, FULL_NAME, OMD_NAME, ONELINE_NAME, MAP_NAME, REOPT_FOLDER_BASE, REOPT_FOLDER_FINAL, BIG_OUT_NAME, QSTS_STEPS, FAULTED_LINE, mg_name, ADD_COST_NAME, FOSSIL_BACKUP_PERCENT, FOSSIL_OVER_CRIT_KW, DIESEL_SAFETY_FACTOR=False, open_results=True, final_run=False):
	...
	# to run microgridup with automatic feedback loop to update fossil size using DIESEL_SAFETY_FACTOR, include the following two lines:
	...
	# to run microgridup with automatic feedback loop to update fossil size using FOSSIL_OVER_CRIT_KW, include the following one line and reset all 18 arguments of "FOSSIL_OVER_CRIT_KW" and "fossil_over_crit_calc" not to "=False":
	fossil_over_crit_calc = fossil_adder('/allOutputData.json', REOPT_FOLDER_BASE, FOSSIL_OVER_CRIT_KW, max_crit_load)
	feedback_reopt_gen_values(BASE_NAME, LOAD_NAME, REOPT_INPUTS, REOPT_FOLDER_BASE, REOPT_FOLDER_FINAL, microgrid, critical_load_percent, fossil_over_crit_calc, diesel_total_calc=False)
	...
	gen_obs = build_new_gen_ob_and_shape(REOPT_FOLDER_FINAL, GEN_NAME, microgrid, BASE_NAME, mg_name, fossil_over_crit_calc, diesel_total_calc=False)
	gen_existing_ref_shapes(REF_NAME, REOPT_FOLDER_BASE, REOPT_FOLDER_FINAL, fossil_over_crit_calc)
	...
	microgrid_report_csv('/allOutputData.json', f'ultimate_rep_{FULL_NAME}.csv', REOPT_FOLDER_FINAL, microgrid, mg_name, max_crit_load, ADD_COST_NAME, fossil_over_crit_calc, diesel_total_calc=False)
	mg_list_of_dicts_full = microgrid_report_list_of_dicts('/allOutputData.json', REOPT_FOLDER_FINAL, microgrid, mg_name, max_crit_load, ADD_COST_NAME, fossil_over_crit_calc, diesel_total_calc=False)
	if final_run:
		# Make OMD.
...
def full(MODEL_DIR, BASE_DSS, LOAD_CSV, QSTS_STEPS, FOSSIL_BACKUP_PERCENT, REOPT_INPUTS, MICROGRIDS, FAULTED_LINE, FOSSIL_OVER_CRIT_KW, DIESEL_SAFETY_FACTOR=False, DELETE_FILES=False, open_results=False, OUTAGE_CSV=None):
	# CONSTANTS
	...
	try:
		with open('allInputData.json','w') as inputs_file:
			inputs = {
				...
				'FOSSIL_OVER_CRIT_KW':FOSSIL_OVER_CRIT_KW,

import os, csv, math, json, logging
...
def estimate_batt_land_use(capacity_mwh):
	return capacity_mwh * 0.3

def estimate_generation_land_use(fossil_kw_ratings, rengen_kw_ratings, batt_kwh_ratings):
	'''
	...
	'''
	fossil_acres = 0
	for key in fossil_kw_ratings: 
		rating_kw = float(fossil_kw_ratings[key])
		# fossil_acres += estimate_fossil_generation_land_use(rating_kw / 1000, diesel_or_natural_gas='diesel') # Uncomment to assume diesel generation
		fossil_acres += estimate_fossil_generation_land_use(rating_kw / 1000, diesel_or_natural_gas='natural_gas') # Uncomment to assume natural gas generation
	rengen_acres = 0
	for key in rengen_kw_ratings: 
		rating_kw = float(rengen_kw_ratings[key])
		if 'solar' in key:
			solar_or_wind = 'solar'
		elif 'wind' in key:
			solar_or_wind = 'wind'
		else: 
			raise ValueError(f'Unknown generation type: {key}.')
		rengen_acres += estimate_rengen_land_use(rating_kw / 1000, solar_or_wind=solar_or_wind)
	batt_acres = 0
	for key in batt_kwh_ratings:
		rating_kwh = float(batt_kwh_ratings[key])
		batt_acres += estimate_batt_land_use(rating_kwh / 1000)
	return fossil_acres + rengen_acres + batt_acres
...
def make_chart(csvName, y_list, year, microgrids, chart_name, y_axis_name, outage_start, outage_end, outage_length, batt_kwh_ratings, fossil_kw_ratings, vsource_ratings={}, rengen_kw_ratings={}, rengen_mgs={}):
	...
	# Estimate land use by generation (do it here because land use by storage is estimated in extract_charting_data()).
	...
	for ob_name in set(gen_data['Name']):
		...
		for y_name in y_list: 
			...
			if ("lead_gen_" in ob_name or "fossil_" in ob_name) and not this_series[y_name].isnull().values.any(): # TODO: get object type with less error prone method.
				...
				natural_gas_consumption_outage_cubic_feet = natural_gas_consumption_outage_mmbtu / 0.001038 # https://www.eia.gov/tools/faqs/faq.php?id=45&t=8
				diesel_storage_area_square_feet = calculate_land_for_diesel_storage(diesel_consumption_outage_gal)
				diesel_storage_area_square_feet = calculate_land_for_diesel_storage_optimized(diesel_consumption_outage_gal)
				cng_storage_area_square_feet = calculate_land_for_natural_gas_storage(natural_gas_consumption_outage_cubic_feet, cng_or_lng='cng')
				lng_storage_area_square_feet = calculate_land_for_natural_gas_storage(natural_gas_consumption_outage_cubic_feet, cng_or_lng='lng')

...
def microgrid_report_csv(inputName, outputCsvName, REOPT_FOLDER, microgrid, mg_name, diesel_total_calc):
	...
	with open(outputCsvName, 'w', newline='') as outcsv:
		...
		row =[str(mg_name), gen_bus_name, round(min_load,0), round(ave_load,0), round(avg_daytime_load,1), round(max_load,0),
...
def microgrid_report_list_of_dicts(inputName, REOPT_FOLDER, microgrid, mg_name, diesel_total_calc):
...
def main(BASE_NAME, LOAD_NAME, REOPT_INPUTS, microgrid, playground_microgrids, GEN_NAME, REF_NAME, FULL_NAME, OMD_NAME, AMPS_NAME, ONELINE_NAME, MAP_NAME, REOPT_FOLDER_BASE, REOPT_FOLDER_FINAL, BIG_OUT_NAME, QSTS_STEPS, DIESEL_SAFETY_FACTOR, mg_name, open_results=True):
	...
	microgridup_control.play(OMD_NAME, BASE_NAME, AMPS_NAME, None, None, playground_microgrids, '670671', False, 60, 120, 30) #TODO: unify the microgrids data structure.
...
def full(MODEL_DIR, BASE_DSS, LOAD_CSV, QSTS_STEPS, DIESEL_SAFETY_FACTOR, REOPT_INPUTS, MICROGRIDS):
	...
	for i, mg_name in enumerate(mgs_name_sorted):
		...
		main(BASE_DSS, MODEL_LOAD_CSV, REOPT_INPUTS, MICROGRIDS[mg_name], MICROGRIDS, GEN_NAME, REF_NAME, f'circuit_plusmg_{i}.dss', OMD_NAME, AMPS_NAME, ONELINE_NAME, MAP_NAME, f'reopt_base_{i}', f'reopt_final_{i}', f'output_full_{i}.html', QSTS_STEPS, DIESEL_SAFETY_FACTOR, mg_name, open_results=False)
	# Build Final report

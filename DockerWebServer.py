...
if not os.path.isdir(os.path.join(TARGET_DIR, PROJ_DIR)):
    ...
    os.chdir(TARGET_DIR)
    subprocess.call(['git','clone','--depth=1','https://github.com/dpinney/microgridup.git'])
else: 
...
process = subprocess.Popen(['docker-compose', '-p', 'mguproj', '-f', '-', 'up','-d'], stdin=subprocess.PIPE, text=True)
process.stdin.write(DOCKER_COMPOSE_FILE)
process.stdin.close()
process.wait()
...
subprocess.call(['read', '-n1', '-r', '-p', 'Server Running at http://localhost:5000. Press any key to stop...', 'key'])
# subprocess.call(['docker', 'stop', 'mgucont'])
